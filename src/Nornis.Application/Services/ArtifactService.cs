...
public class ArtifactService : IArtifactService
{
        ...
        Guid worldId, Guid requestingUserId, WorldRole role, CancellationToken ct)
    {
        ...
        // Every campaign in the world by id, so a lane's campaigns resolve to a name and a
        // declared start date.
        ...
        var lanes = storylines
            .Select(s =>
            {
                ...
                // A storyline spans every campaign its dated sessions fall in — derived, never
                // declared, so it cannot disagree with the sessions it is drawn from. Orphaned
                // ids (a campaign since deleted) simply drop out.
                var campaignIds = points.Select(p => p.CampaignId).OfType<Guid>().ToHashSet();
                ...
                // lane spans, which is the head of that ordering. Never a vote, so a
                // cross-campaign arc lands in a stable, explainable band.
                var anchor = campaigns.FirstOrDefault();
