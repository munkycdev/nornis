...
/// <summary>
/// Integration tests for GET /api/campaigns/{campaignId}/sources/{sourceId}
...
public class SourceGetTests
{
    ...
    public async Task NonExistentSource_Returns404()
    {
        ...
        // Act
        var response = await _scenario.GmClient.GetAsync(
            $"/api/campaigns/{_scenario.Campaign.Id}/sources/{nonExistentSourceId}");
    ...
    /// <summary>
    /// A non-member of the campaign cannot access source endpoints — returns 403.
    /// Validates: Requirements 2.6, 10.1, 10.2
    /// </summary>
    [Test]
    public async Task NonMember_CannotGetSource_Returns403()
    {
        // Arrange — Create a source in the campaign
        var source = await SourceTestHelpers.CreateTestSourceAsync(
            _factory,
            _scenario.Campaign.Id,
            _scenario.GmUserId,
        ...
        // Create a separate user who is NOT a member of this campaign
        ...
        await nonMemberClient.GetAsync("/api/campaigns");

        // Act — Non-member attempts to retrieve the source
        var response = await nonMemberClient.GetAsync(
            $"/api/campaigns/{_scenario.Campaign.Id}/sources/{source.Id}");

        // Assert — 403 Forbidden because user is not a campaign member
