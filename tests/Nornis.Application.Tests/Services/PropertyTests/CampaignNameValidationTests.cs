...
/// both campaign creation and campaign update operations should reject the input with a validation error
/// (status 400).
///
/// **Validates: Requirements 3.3, 4.6**
/// </summary>
[TestFixture]
[Category("Feature: auth-and-campaigns, Property 5: Campaign Name Validation Rejects Invalid Names")]
public class CampaignNameValidationTests
{
    [FsCheck.NUnit.Property(
        Arbitrary = [typeof(InvalidNameArbitraries)],
        MaxTest = 100)]
    [Description("Feature: auth-and-campaigns, Property 5: Campaign Name Validation Rejects Invalid Names - CreateAsync rejects invalid names")]
    public void CreateAsync_RejectsInvalidNames(InvalidNameInput input)
    {
        ...
        Assert.That(result.IsSuccess, Is.False,
            $"Campaign creation should reject invalid name: \"{input.InvalidName ?? "(null)"}\"");
    ...
    [FsCheck.NUnit.Property(
        Arbitrary = [typeof(InvalidNameArbitraries)],
        MaxTest = 100)]
    [Description("Feature: auth-and-campaigns, Property 5: Campaign Name Validation Rejects Invalid Names - UpdateAsync rejects invalid names")]
    public void UpdateAsync_RejectsInvalidNames(InvalidNameInput input)
    {
        // Arrange - first create a valid campaign as GM
        var campaignRepo = new InMemoryCampaignRepository();
        var memberRepo = new InMemoryCampaignMemberRepository();
        var service = new CampaignService(campaignRepo, memberRepo);
        ...
        Assert.That(createResult.IsSuccess, Is.True, "Setup: valid campaign creation should succeed.");
        ...
        Assert.That(updateResult.IsSuccess, Is.False,
            $"Campaign update should reject invalid name: \"{input.InvalidName ?? "(null)"}\"");
...
/// <summary>
/// Input model for campaign name validation property tests.
/// </summary>
...
/// <summary>
/// Custom FsCheck arbitraries for invalid campaign name generation.
