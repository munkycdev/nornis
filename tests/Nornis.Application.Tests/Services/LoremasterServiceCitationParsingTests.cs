...
public class LoremasterServiceCitationParsingTests
{
    ...
    public async Task AskAsync_AnswerTextPreservesCitationMarkersForUiRendering()
    {
        ...
        var service = new LoremasterService(knowledgeRetriever, new FakeReferencePassageRetriever(), aiClient, TestUsageRecorder.Wrap(usageRepository), new FakeAiBudgetGuard(), options, NullLogger<LoremasterService>.Instance);
